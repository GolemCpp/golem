...
def package_dmg(self, package_build_context):
    ...
    package_version = '{}.{}.{}'.format(version.major, version.minor, version.patch)
    package_build_version = '{}.{}.{}.{}'.format(
    ...
    app_directory = os.path.join(package_directory, app_bundle_name, 'Contents')
    ...
    if dmg_package.skeleton:
        package_skeleton = os.path.join(self.get_project_dir(), dmg_package.skeleton)
    ...
    for artifact in artifacts:
        ...
        dst_directory = os.path.realpath(
            helpers.make_directory(
                subdirectory_directory, os.path.join(local_dir, artifact_dirname)
            )
        )
        ...
        artifact.path = os.path.join(local_dir, artifact_dirname, artifact_filename)
    ...
    for template_file in template_files:
        ...
        template_file_relative = os.path.relpath(
            path=template_file, start=app_directory
        )
        ...
        self.context(
            ...
            GOLEM_PACKAGE_DMG_BUNDLE_EXECUTABLE=str(
                os.path.relpath(
                    path=os.path.join(app_directory, target_programs[0]),
    ...
    if self.is_qt_enabled(package_build_context.configuration):
        ...
        for binary in unique_targets_binaries:
            ...
            deployqt_bin = os.path.join(self.context.env.QT_HOST_BINS, 'macdeployqt')
            helpers.run_task(
                [
                    deployqt_bin,
                    app_bundle_name,
        ...
        qt_conf_path = os.path.join(subdirectory_directory, 'Resources', 'qt.conf')
    ...
    if dmg_package.background:
        volume_background_path = os.path.join(
            self.get_project_dir(), dmg_package.background
        )
    ...
    background_script_line = (
        'set background picture of viewOptions to file ".background:{}"'.format(
            os.path.basename(volume_background_path)
        )
...
""".format(volume_name, background_script_line, app_bundle_name)
    ...
    ps3 = subprocess.Popen(
        ('sed', '1q'), stdin=ps2.stdout, stdout=subprocess.PIPE, cwd=package_directory
    )
    device_id = (
        subprocess.check_output(
            ['awk', '{print $1}'], stdin=ps3.stdout, cwd=package_directory
        )
        .split()[0]
        .decode('utf-8')
    )
    ...
    if volume_background_path:
        print(
            "Install background image into the DMG volume: {}".format(
                volume_background_path
            )
        )
        background_directory = os.path.join("/Volumes", volume_name, ".background")
    ...
    print("{}".format(script))
    ps = subprocess.Popen(
        ('echo', script), stdout=subprocess.PIPE, cwd=package_directory
    )
    _ = subprocess.check_output(['osascript'], stdin=ps.stdout, cwd=package_directory)
    ps.wait()

    helpers.run_task(["sync"], cwd=package_directory, debug=True)

    helpers.run_task(
        ['hdiutil', 'detach', device_id], cwd=package_directory, debug=True
    )
    ...
    for artifact in artifacts:
        artifact_file = File(
            path=artifact.path, absolute_path=artifact.absolute_path, type=artifact.type
        )
    ...
    package_file = File(
        path=volume_final,
        absolute_path=os.path.join(package_directory, volume_final),
        type='package',
    )
