...
def package_msi(self, package_build_context):
    ...
    if msi_package.skeleton:
        package_skeleton = os.path.join(self.get_project_dir(), msi_package.skeleton)
    ...
    if msi_package.project:
        package_wix_directory = os.path.join(
            self.get_project_dir(), msi_package.project
        )

    if package_wix_directory and not os.path.exists(package_wix_directory):
        raise RuntimeError(
            "Package wix directory doesn't exist: {}".format(package_wix_directory)
        )
    ...
    msvc_crt_merge = (
        '<Merge Id="VCRedist" SourceFile="{}" DiskId="1" Language="0"/>'.format(
            msvc_crt_path
        )
    )
    ...
    for wix_file in wxs_files + wxl_files:
        ...
        if not os.path.exists(str(wix_file)):
            raise RuntimeError("Cannot find any file at {}".format(str(wix_file)))
        ...
        self.context(
            ...
            GOLEM_PACKAGE_MSI_INSTALL_FILES_COMPONENT_REFS=str(component_refs_string),
