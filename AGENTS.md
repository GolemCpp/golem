...
- **A line is at most 88 characters**, which is what `[tool.ruff]` in [pyproject.toml](pyproject.toml) sets and `E501` is selected to check, alone. Most of the code sits well under it and the lines already over are fixed as they are touched, therefore what a change is measured against is `ruff check --force-exclude $(git diff --name-only origin/develop... -- '*.py')` rather than the whole tree.
...
- **Start a function or method docstring with a verb, in the imperative**, which is what PEP 257 asks for: `Make the cache key of an item.`, not `Makes the cache key ...` and not `The key identifying an item ...`. Say what calling it does, then what the result means.
  ...
  - A predicate may be written as the question it answers: `Does the version name a semver range or is it a ref?`. Only one that _only_ answers. One that also does work names the work and returns the answer (`Convert what the root holds into what is asked for now, and return whether it can keep being used.`), because a question-shaped name promises no side effects.
  - Name the thing the verb acts on: `Refuse an identity in a field taking a locator.`, not `Refuse a field taking a locator that was given an identity.` — a field is not what gets refused. An imperative docstring has no subject, therefore its object is the only noun carrying the meaning.
...
- **Write plainly.** Name the subject, then say what follows from it.
  ...
  - Do not let a modifier between the subject and its verb end on a noun the verb can swallow: `A recipe declaring mirrors and no locator names no default remote.` garden-paths on `locator names`, so the reader reaches `no default remote` and backs up. Write `A recipe names no default remote when it declares mirrors and no locator.` A short modifier is fine when no noun sits against the verb, as in `an identity naming no host contradicts nothing`.
