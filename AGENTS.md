...
- [src/golemcpp/golem/resource_manager.py](src/golemcpp/golem/resource_manager.py): how every resource kind (dependency, cookbook, overlay, tool) is fetched into the cache. The mechanism is shared and the richest a kind needs — shallow clones, submodules, cleaning, checkout-then-reset — so a kind asks for what it wants through the `FetchPolicy` it builds rather than writing its own git. A kind defines only data — `source_for`, `source_path`, `policy_for` — plus `resolve_version` and the lifecycle `install` brackets the fetch with: `pre_install`, `pre_install_refresh` and `post_install`, which is where a kind that makes something from its source (a tool builds its binary) says so. `resolve_version` runs from `resolve_cached_resource`, because what a version resolves to is part of what identifies a resource: nothing can ask where one lives before it is resolved. `CacheManager.guard_install`/`guard_refresh` are what keep a resource's content and the manifest naming it from ever disagreeing.
