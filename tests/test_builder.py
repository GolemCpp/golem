...
def make_recording_context(observed, directory=''):
    ...
    '''
    def record(step):
        observed.append((step, network.is_allowed(),
                         bool(advertisement_store.directory_in_use())))
...
def test_resolve_keeps_the_advertisements_under_the_build_directory(monkeypatch, tmp_path):
    observed = []
...
def test_build_and_dependencies_read_what_resolve_put_in_the_cache(monkeypatch):
    ...
    assert observed == [
        ('environment', False, False), ('build', False, False),
        ('environment', False, False), ('dependencies', False, False),
    ]
