...
from golemcpp.golem.cookbook_manager import CookbookManager, get_cookbook_manager
...
def make_source():
    return RequestedSource.for_repository(
        "https://example.com/recipes.git", version="main"
    )
...
def test_making_cookbooks_available_keeps_the_configured_order(tmp_path):
    manager = make_manager(tmp_path)
    sources = [
        RequestedSource.for_repository(
            "https://example.com/{}.git".format(name), version="main"
        )
