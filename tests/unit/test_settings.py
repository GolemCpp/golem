...
def read_clean_env(name, prefix=""):
    ...
    return [
        ...
        for line in lines
        if line.startswith(prefix + "GOLEM_")
    ]
...
def test_prefers_option_then_environment_then_store(monkeypatch, tmp_path):
    ...
    config_store.set_value(
        "cache.directory", store_cache, config_store.GLOBAL_SCOPE, project_dir
    )
    monkeypatch.delenv("GOLEM_CACHE_DIRECTORY", raising=False)

    def resolve(options=None):
        return (
            settings.get_settings(options=options, project_dir=project_dir)
            .get("GOLEM_CACHE_DIRECTORY")
            .location
        )
...
def test_reads_persisted_configure_options(monkeypatch, tmp_path):
    ...
    assert (
        settings.get_settings(build_dir=build_dir).get("GOLEM_CACHE_DIRECTORY").location
        == configured_cache
    )
    assert (
        settings.get_settings(
            options=SimpleNamespace(cache_directory=cli_cache), build_dir=build_dir
        )
        .get("GOLEM_CACHE_DIRECTORY")
        .location
        == cli_cache
...
def test_converts_to_the_setting_type(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv(
        "GOLEM_ADDITIONAL_CACHE_DIRECTORIES", "{}|{}=github".format(first, second)
    )
    ...
    assert [
        (entry.location, entry.regex, entry.is_read_only)
        for entry in manager.get("GOLEM_ADDITIONAL_CACHE_DIRECTORIES")
    ] == [(first, None, False), (second, "github", False)]


def test_reports_an_unparsable_value_with_the_source_it_comes_from(
...
def test_reports_the_option_and_the_store_an_unparsable_value_comes_from(
    monkeypatch, tmp_path
):
    _isolate_home(monkeypatch, tmp_path)
    monkeypatch.delenv("GOLEM_CACHE_MINIMIZATION_LENGTH", raising=False)
    ...
    with pytest.raises(ValueError, match="from option --cache-minimization-length"):
        settings.get_settings(
            options=SimpleNamespace(cache_minimization_length="not-a-number")
        ).get("cache.minimization.length")

    with pytest.raises(
        ValueError, match="from local configuration key cache.minimization.enabled"
    ):
...
def test_one_manager_answers_every_setting(monkeypatch, tmp_path):
    ...
    assert (
        manager.get("GOLEM_CACHE_DIRECTORY").location
        == settings.get_default_cache_directory_path()
    )
...
def test_a_location_setting_names_the_source_of_a_bad_kind(monkeypatch, tmp_path):
    ...
    with pytest.raises(ValueError) as error:
        settings.get_settings(project_dir=str(tmp_path)).get(
            "GOLEM_COOKBOOKS_LOCATIONS"
        )
...
def test_parse_value_refuses_a_bad_location_where_it_is_written(monkeypatch, tmp_path):
    ...
    assert [
        source.type
        for source in manager.parse_value("cookbooks.locations", "https://host/r.git")
    ] == ["git"]
...
def test_legacy_names_keep_resolving(monkeypatch, tmp_path):
    ...
    config_store.set_value(
        "cache.dir", "/legacy/store/cache", config_store.LOCAL_SCOPE, project_dir
    )
    assert manager.get(setting) == "/legacy/store/cache"

    # The current spelling still wins over the legacy one.
    config_store.set_value(
        "cache.directory", "/current/cache", config_store.LOCAL_SCOPE, project_dir
    )
...
def test_a_path_setting_resolves_against_the_project(monkeypatch, tmp_path):
    ...
    assert manager.get("GOLEM_CACHE_DIRECTORY").location == os.path.join(
        project_dir, "local-cache"
    )
    assert manager.get("GOLEM_OVERRIDES_CONFIGURATION") == os.path.join(
        project_dir, "overrides.json"
    )
...
def test_make_flag_spells_a_value_the_way_a_command_reads_it(monkeypatch, tmp_path):
    ...
    assert manager.make_flag("GOLEM_CACHE_DIRECTORY") == [
        "--cache-directory={}".format(cache_dir)
    ]
    assert manager.make_flag("GOLEM_CACHE_MINIMIZATION_ENABLED") == [
        "--cache-minimization-enabled=off"
    ]
    assert manager.make_flag("GOLEM_CACHE_MINIMIZATION_LENGTH") == [
        "--cache-minimization-length=16"
    ]
    assert manager.make_flag("GOLEM_CACHE_RESOLUTION_POLICY") == [
        "--cache-resolution-policy=strict"
    ]
...
def test_make_flag_repeats_a_list_setting_and_absolutizes_it(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv(
        ...
        "shared=github|{}".format(cache_dir),
    )
    manager = settings.get_settings(project_dir=project_dir)

    assert manager.make_flag("GOLEM_ADDITIONAL_READ_ONLY_CACHE_DIRECTORIES") == [
        "--additional-read-only-cache-directory={}=github".format(
...
def test_the_resolution_policy_resolves_to_its_enum(monkeypatch, tmp_path):
    ...
    def resolve(options=None):
        return settings.get_settings(options=options, project_dir=project_dir).get(
            "GOLEM_CACHE_RESOLUTION_POLICY"
        )
    ...
    assert (
        resolve(options=SimpleNamespace(cache_resolution_policy="weak"))
        == CacheResolutionPolicy.WEAK
    )
    # And it goes back out as the text a sub-build parses.
    assert settings.get_settings(
        options=SimpleNamespace(cache_resolution_policy="weak"), project_dir=project_dir
    ).make_flag("GOLEM_CACHE_RESOLUTION_POLICY") == ["--cache-resolution-policy=weak"]
...
def test_get_default_processes_the_built_in_default(monkeypatch, tmp_path):
    ...
    assert (
        manager.get_default("GOLEM_CACHE_RESOLUTION_POLICY")
        == CacheResolutionPolicy.STRICT
    )
...
def test_the_job_count_reads_back_from_every_source(monkeypatch, tmp_path):
    ...
    # And it goes back out as the text a sub-build parses.
    assert settings.get_settings(
        options=SimpleNamespace(git_jobs=12), project_dir=project_dir
    ).make_flag("GOLEM_GIT_JOBS") == ["--git-jobs=12"]
...
def test_whether_git_may_prompt_reads_back_from_every_source_it_has(
    monkeypatch, tmp_path
):
    _isolate_home(monkeypatch, tmp_path)
    ...
    def resolve():
        return settings.get_settings(project_dir=project_dir).get(
            "GOLEM_GIT_PROMPT_ENABLED"
        )
