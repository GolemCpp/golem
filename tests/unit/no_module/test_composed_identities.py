...
LOCATOR_IDENTITIES = [
    ...
    ('@git@pub.scm.git@git.kernel.org', ['git://git.kernel.org/pub/scm/git/git.git']),
    ('@proj@group.subgroup@gitlab.com', ['https://gitlab.com/group/subgroup/proj.git']),
    (
        '@repo@@host.xz',
        [
    ...
    ('@repo@o@host.example.com=d15741ed', ['ssh://host.example.com:99999/o/repo.git']),
...
ADVERTISEMENT_IDENTITIES = [
    ...
    (
        '@json@nlohmann@github.com',
        ['https://github.com/nlohmann/json.git', 'https://github.com/nlohmann/json'],
    ),
...
@pytest.mark.parametrize(
    'kind, cache_key, at_eight, at_sixteen',
    [
        (
            ResourceKind.DEPENDENCY,
            '@json@nlohmann@github.com#65ee684',
            'a1427f16',
            'a1427f162264de1a',
        ),
        (
            ResourceKind.COOKBOOK,
...
def test_make_minimized_resource_name_names(
    tmp_path, kind, cache_key, at_eight, at_sixteen
):
...
def test_make_minimized_resource_name_separates_kinds(tmp_path):
    ...
    names = {
        manager.make_minimized_resource_name(
...
def test_a_dependency_list_composes_its_identity(dependencies, identity):
    ...
    assert context.make_dependencies_slug(make_dependencies(*dependencies)) == identity
